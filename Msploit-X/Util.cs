...
namespace Msploit_X
{
    public class Util
    {
        ...
        public static Result updateHostFromScan(string resultFile)
        {
            ...
            try
            {
                using (var xmlStream = new StreamReader(resultFile + ".xml"))
                {
            ...
            return new Result() {Hosts = hosts, ScanName = DateTime.Now.ToShortTimeString(), OutputText = File.ReadAllText(resultFile + ".txt")};
        }
    }
}